export const environment = {
  production: false,
  apiItemIdUrl: 'https://www.themealdb.com/api/json/v1/1/lookup.php?i=',
  apiUrlRandom: 'https://www.themealdb.com/api/json/v1/1/random.php',
  apiItemsLetterUrl: 'https://www.themealdb.com/api/json/v1/1/search.php?f=',
  apiSearchByNameUrl: 'https://www.themealdb.com/api/json/v1/1/search.php?s=',
  urlImageIngredient: 'https://www.themealdb.com/images/ingredients/',
  apiSearchByCategory: 'https://www.themealdb.com/api/json/v1/1/filter.php?c=',
  apiSearchByArea: 'https://www.themealdb.com/api/json/v1/1/filter.php?a=',
  apiSearchByIngredient: 'https://www.themealdb.com/api/json/v1/1/filter.php?i=',
  apiListAllIngredients: 'https://www.themealdb.com/api/json/v1/1/list.php?i=list',
};
